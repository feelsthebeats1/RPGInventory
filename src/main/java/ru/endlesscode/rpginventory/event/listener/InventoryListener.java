...
public class InventoryListener implements Listener {
    ...
    public void onPickupToQuickSlot(@NotNull EntityPickupItemEvent event) {
        ...
        if (!InventoryManager.playerIsLoaded(player) || itemStack == null || !ItemManager.allowedForPlayer(player, itemStack, false)) {
            return;
        }

        for (Slot quickSlot : SlotManager.instance().getQuickSlots()) {
            int slotId = quickSlot.getQuickSlot();
            if (quickSlot.isCup(player.getInventory().getItem(slotId)) && quickSlot.isValidItem(itemStack)) {
